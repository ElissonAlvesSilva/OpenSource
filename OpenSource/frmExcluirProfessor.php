...
{
    $registro_academico = new \Frequencia\Models\Registro_Academico;
    $user = new \Frequencia\Models\Usuario_RM;
    $t = new \Frequencia\Models\Turma;
    $professor = $prof->findByType('idProfessor',$id);
    $turma = $t->findByType('Professor_idProfessor',$id);
    if($turma == false){

    } else{
        $t->update($turma->idTurma,['Professor_idProfessor'=>null],'idTurma');
    }
    $user->delete('RA',$professor->RA);
    $ra_prof = $professor->RA;

    $prof->delete('idProfessor',$id);

    $registro_academico->delete('idRegistro',$ra_prof);
                          ...
                          <div class="modal-dialog">
                            <div class="modal-content modal-window">
                              ...
                              <div class="modal-body">
                                <p> Professor <b><i>'.$_POST['nome'].'</i></b> removido com sucesso</p>
                              </div>
                              <div class="modal-footer">
                                <button type="button" class="btn btn-default" data-dismiss="modal" id="button_remove_prof">Fechar</button>
                              </div>
                            </div><!-- /.modal-content -->
                          </div><!-- /.modal-dialog -->
                        </div><!-- /.modal -->
